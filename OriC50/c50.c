...
int select_rhs_if_lhs_is_positive(int lhs, int rhs) {
    ...
    fprintf(Of, "lhs is %d, rhs is %d , select when positive.\n", lhs, rhs);
...
int select_rhs_if_lhs_is_negative(int lhs, int rhs) {
    ...
    // Consistency check of the sample
    fprintf(Of, "rhs is %d, lhs is %d , select when negative.\n", rhs, lhs);
    if (Class(Case[lhs]) == 2 && Class(Case[rhs]) == 1) {
        fprintf(Of, "Found inconsistency in sample, exiting");
        exit(1);
    }
...
/*  ----  */
{
    ...
    */
    ...
    if (!(F = GetFile(".dup.data", "r"))) Error(NOFILE, "", "");
#if false
    ...
    fprintf(Of, "Start propagate from left to right: \n");
    ...
    fprintf(Of, "Start propagate from right to left: \n");
