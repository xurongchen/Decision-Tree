...
void delete_cmap(cmap *map) {
    ...
    if (map != NULL) {
        ...
        for (std::map<int, std::set<int> *>::iterator it = map->left2right_map.begin();
             it != map->left2right_map.end(); it++) {
            delete it->second;
        }

        for (std::map<int, std::set<int> *>::iterator it = map->right2left_map.begin();
             it != map->right2left_map.end(); it++) {
            delete it->second;
        }
...
cmap *cmap_copy(cmap const *other) {
    ...
    // Copy left2right map
    for (std::map<int, std::set<int> *>::const_iterator it1 = other->left2right_map.begin();
         it1 != other->left2right_map.end(); it1++) {
    ...
    // Copy right2left map
    for (std::map<int, std::set<int> *>::const_iterator it1 = other->right2left_map.begin();
         it1 != other->right2left_map.end(); it1++) {
...
void AddImplForRangeWhenSplit(struct cmap *map,int Op, int Np, struct array const *permutation){
    if(map == NULL || permutation == NULL)
        return;
    std::map<int, std::set<int> *>::iterator it = map->left2right_map.find(Op);

    if(it != map->left2right_map.end()){
        if(it->second == NULL)
            map->left2right_map[permutation->entries[Np]] = NULL; // ,maybe can just use Np, not permutation
        else{
            for (std::set<int>::const_iterator it2 = it->second->begin(); it2 != it->second->end(); it2++) {
                assert(0 <= *it2 && *it2 < permutation->size);
                cmap_add(map, Np, *it2);
                //cmap_add(map, Np, permutation->entries[*it2]);
            }
        }

    }

    if((it = map->right2left_map.find(Op)) != map->right2left_map.end()){
        if(it->second == NULL)
            map->right2left_map[permutation->entries[Np]] = NULL; // ,maybe can just use Np, not permutation
        else{
            for (std::set<int>::const_iterator it2 = it->second->begin(); it2 != it->second->end(); it2++) {
                assert(0 <= *it2 && *it2 < permutation->size);
                cmap_add(map,*it2, Np);
...
struct array *cmap_get_keys(struct cmap const *map) {
    ...
    int i = 0;
    for (std::map<int, std::set<int> *>::const_iterator it = map->left2right_map.begin();
         it != map->left2right_map.end(); it++) {
...
int cmap_count_cuts(struct cmap const *m, struct array const *a1, struct array const *a2) {
    ...
    // Now count
    int count = 0;
    for (std::map<int, std::set<int> *>::const_iterator it1 = m->left2right_map.begin();
         it1 != m->left2right_map.end(); it1++) {
        for (std::set<int>::const_iterator it2 = it1->second->begin(); it2 != it1->second->end(); it2++) {
...
int cmap_number_of_implications(cmap const *map) {
    ...
    int number = 0;
    for (std::map<int, std::set<int> *>::const_iterator it1 = map->left2right_map.begin();
         it1 != map->left2right_map.end(); it1++) {
...
void cmap_print(cmap *map) {
    ...
    for (std::map<int, std::set<int> *>::iterator it1 = map->left2right_map.begin();
         it1 != map->left2right_map.end(); it1++) {
    ...
    for (std::map<int, std::set<int> *>::iterator it1 = map->right2left_map.begin();
         it1 != map->right2left_map.end(); it1++) {
        out << std::endl << "\t" << it1->first << " <- [";
