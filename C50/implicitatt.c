...
typedef  union  _xstack_elt_value
{
    DiscrValue	_discr_val;
    ContValue	_cont_val;
    ContRangeVal _contran_val;
    String _string_val;
}XStackEltValue;

typedef struct _xstack_elt {
    AttType _elt_type;
    XStackEltValue _elt_value;
} XStackElt;

#define    cval        _cont_val
#define    sval        _string_val
#define    dval        _discr_val
#define    crval       _contran_val
#define    elval       _elt_value
...
/*									 */
/*************************************************************************/
#define    CRUnknownVal(AV)       (AV.crval._s_val == _UNK.cval || AV.crval._e_val == _UNK.cval)
#define    CUnknownVal(AV)        (AV.cval==_UNK.cval)
#define    DUnknownVal(AV)        (AV.dval==_UNK.dval)
#define DUNA(a)    (DUnknownVal(XStack[a]._elt_value) || NotApplicVal(XStack[a]._elt_value))
#define CUNA(a)    (CUnknownVal(XStack[a]._elt_value) || NotApplicVal(XStack[a]._elt_value))
...
ContValue MaxContValue(int count, ...){
    if(count <= 0)
        return 0;
    va_list ap;

    va_start(ap, count);
    ContValue  maxValue = FLT_MIN;
    for(int i = 0; i < count; ++i) {
        ContValue parameter = va_arg(ap, ContValue);
        maxValue = parameter >= maxValue ? parameter : maxValue;
    }
    va_end(ap);
    return maxValue;
}

ContValue MinContValue(int count, ...){
    if(count <= 0)
        return 0;
    va_list ap;

    va_start(ap, count);
    ContValue  minValue = FLT_MAX;
    for(int i = 0; i < count; ++i) {
        ContValue parameter = va_arg(ap, ContValue);
        minValue = parameter <= minValue ? parameter : minValue;
    }
    va_end(ap);
    return minValue;
}
    ...
    for (DN = 0;; DN++) {
        switch (DefOp((DElt = D[DN]))) {
            case OP_ATT:
                ...
                if (Continuous(Att)) {
                    if(TypeVal(Case, Att) == MYCONTRANGE){
                        XStack[XSN]._elt_value._contran_val._s_val = CRSVal(Case, Att);
                        XStack[XSN]._elt_value._contran_val._e_val = CREVal(Case, Att);
                        XStack[XSN++]._elt_type = MYCONTRANGE;
                    }else {
                        XStack[XSN]._elt_value.cval = CVal(Case, Att);
                        XStack[XSN++]._elt_type = MYCONT;
                    }
                } else {
                    XStack[XSN]._elt_value.sval =
                            (Unknown(Case, Att) && !NotApplic(Case, Att) ? 0 :
                             AttValName[Att][XDVal(Case, Att)]);
                    XStack[XSN++]._elt_type = MYSTRING;
                }
                break;

            case OP_NUM:
                XStack[XSN++]._elt_value.cval = DefNVal(DElt);
                break;

            case OP_STR:
                XStack[XSN++]._elt_value.sval = DefSVal(DElt);
                break;

            case OP_AND:
                bv1 = XStack[XSN - 2]._elt_value.dval;
                bv2 = XStack[XSN - 1]._elt_value.dval;
                XStack[XSN - 2]._elt_value.dval = (bv1 == 3 || bv2 == 3 ? 3 :
                                        D2(bv1 == 2 && bv2 == 2 ? 2 : 3));
                XSN--;
                break;
            ...
            case OP_OR:
                bv1 = XStack[XSN - 2]._elt_value.dval;
                bv2 = XStack[XSN - 1]._elt_value.dval;
                XStack[XSN - 2]._elt_value.dval = (bv1 == 2 || bv2 == 2 ? 2 :
                                        D2(bv1 == 2 || bv2 == 2 ? 2 : 3));
                XSN--;
                break;

            case OP_EQ:
                cv1 = XStack[XSN - 2]._elt_value.cval;
                cv2 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 2]._elt_value.dval = (cv1 == cv2 ? 2 : 3);
                XSN--;
                break;
            ...
            case OP_GT:
                cv1 = XStack[XSN - 2]._elt_value.cval;
                cv2 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 2]._elt_value.dval = CD2(cv1 > cv2 ? 2 : 3);
                XSN--;
                break;
            ...
            case OP_SEQ:
                sv1 = XStack[XSN - 2]._elt_value.sval;
                sv2 = XStack[XSN - 1]._elt_value.sval;
                XStack[XSN - 2]._elt_value.dval =
                        (!sv1 && !sv2 ? 2 :
                         !sv1 || !sv2 ? 3 :
                         !strcmp(sv1, sv2) ? 2 : 3);
                XSN--;
            ...
            case OP_PLUS:
                if(XStack[XSN - 2]._elt_type == MYCONTRANGE || XStack[XSN - 1]._elt_type == MYCONTRANGE){
                    if(XStack[XSN - 2]._elt_type == MYCONTRANGE){
                        cv1 = XStack[XSN - 2]._elt_value._contran_val._s_val;
                        cv2 = XStack[XSN - 2]._elt_value._contran_val._e_val;
                    }else{
                        cv1 = cv2 = XStack[XSN - 2]._elt_value._cont_val;
                    }
                    if(XStack[XSN - 1]._elt_type == MYCONTRANGE){
                        cv3 = XStack[XSN - 1]._elt_value._contran_val._s_val;
                        cv4 = XStack[XSN - 1]._elt_value._contran_val._e_val;
                    }else{
                        cv3 = cv4 = XStack[XSN - 1]._elt_value._cont_val;
                    }
                    XStack[XSN - 2]._elt_value._contran_val._s_val = C2(cv1 + cv3);
                    XStack[XSN - 2]._elt_value._contran_val._e_val = C2(cv2 + cv4);
                    XStack[XSN - 2]._elt_type = MYCONTRANGE;
                }else{
                    cv1 = XStack[XSN - 2]._elt_value.cval;
                    cv2 = XStack[XSN - 1]._elt_value.cval;
                    XStack[XSN - 2]._elt_value.cval = C2(cv1 + cv2);
                    XStack[XSN - 2]._elt_type = MYCONT;
                }
                XSN--;
                break;
            ...
            case OP_MINUS:
                if(XStack[XSN - 2]._elt_type == MYCONTRANGE || XStack[XSN - 1]._elt_type == MYCONTRANGE){
                    ...
                    XStack[XSN - 2]._elt_value._contran_val._s_val = C2(cv1 - cv4);
                    XStack[XSN - 2]._elt_value._contran_val._e_val = C2(cv2 - cv3);
                    XStack[XSN - 2]._elt_type = MYCONTRANGE;
                }else{
                    cv1 = XStack[XSN - 2]._elt_value.cval;
                    cv2 = XStack[XSN - 1]._elt_value.cval;
            ...
            case OP_MULT:
                // NOt implement for the range's multiplication
                if(XStack[XSN - 2]._elt_type == MYCONTRANGE || XStack[XSN - 1]._elt_type == MYCONTRANGE){
                    if(XStack[XSN - 2]._elt_type == MYCONTRANGE){
                        cv1 = XStack[XSN - 2]._elt_value._contran_val._s_val;
                        cv2 = XStack[XSN - 2]._elt_value._contran_val._e_val;
                    ...
                    XStack[XSN - 2]._elt_value._contran_val._s_val = C2(MinContValue(4, cv1*cv3, cv1*cv4, cv2*cv3, cv2*cv4));
                    XStack[XSN - 2]._elt_value._contran_val._e_val = C2(MaxContValue(4, cv1*cv3, cv1*cv4, cv2*cv3, cv2*cv4));
                    XStack[XSN - 2]._elt_type = MYCONTRANGE;
                }else{
                    cv1 = XStack[XSN - 2]._elt_value.cval;
                    cv2 = XStack[XSN - 1]._elt_value.cval;
            ...
            case OP_DIV:
                // NOt implement for the range's div
                /*  Note: have to set precision of result  */

                cv1 = XStack[XSN - 2]._elt_value.cval;
                cv2 = XStack[XSN - 1]._elt_value.cval;
                if (!cv2 ||
                    CUnknownVal(XStack[XSN - 2]._elt_value) ||
                    CUnknownVal(XStack[XSN - 1]._elt_value) ||
                    NotApplicVal(XStack[XSN - 2]._elt_value) ||
                    NotApplicVal(XStack[XSN - 1]._elt_value)) {
                    XStack[XSN - 2]._elt_value.cval = _UNK.cval;
                } else {
                    ...
                    XStack[XSN - 2]._elt_value.cval = rint(cv1 * Mult) / Mult;
                }
                XSN--;
                break;

            case OP_MOD:
                // NOt implement for the range's mod
                cv1 = XStack[XSN - 2]._elt_value.cval;
                cv2 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 2]._elt_value.cval = C2(fmod(cv1, cv2));
                XSN--;
                break;
            ...
            case OP_UMINUS:
                cv1 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 1]._elt_value.cval = C1(-cv1);
                break;

            case OP_SIN:
                cv1 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 1]._elt_value.cval = C1(sin(cv1));
                break;

            case OP_COS:
                cv1 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 1]._elt_value.cval = C1(cos(cv1));
                break;

            case OP_TAN:
                cv1 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 1]._elt_value.cval = C1(tan(cv1));
                break;

            case OP_LOG:
                cv1 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 1]._elt_value.cval =
                        (CUNA(XSN - 1) || cv1 <= 0 ? _UNK.cval : log(cv1));
                break;

            case OP_EXP:
                cv1 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 1]._elt_value.cval = C1(exp(cv1));
                break;

            case OP_INT:
                cv1 = XStack[XSN - 1]._elt_value.cval;
                XStack[XSN - 1]._elt_value.cval = C1(rint(cv1));
                break;

            case OP_END:
                if(XStack[0]._elt_type == MYCONT)
                    ReturnVal._cont_val = XStack[0]._elt_value.cval;    /* cval >= dval bytes */
                else if(XStack[0]._elt_type == MYCONTRANGE)
                    ReturnVal._contran_val = XStack[0]._elt_value._contran_val;
                else{
                    ReturnVal._discr_val = XStack[0]._elt_value._discr_val;
                }
                return ReturnVal;
        }
    }
...
void EvaluateDefNew(Definition D, DataRec *Case, Attribute DefAtt)
/*       -----------  */
{
    XStackElt XStack[100];            /* allows 100-level nesting  */
    int XSN = 0, DN, bv1, bv2, Mult;
    ...
    for (DN = 0;; DN++) {
        switch (DefOp((DElt = D[DN]))) {
            ...
            case OP_END:
                if(XStack[0]._elt_type == MYCONT) {
                    ReturnVal._cont_val = XStack[0]._elt_value.cval;    /* cval >= dval bytes */
                    (*Case)[DefAtt]._attr_type = MYCONT;
                }
                else if(XStack[0]._elt_type == MYCONTRANGE){
                    ReturnVal._contran_val = XStack[0]._elt_value._contran_val;
                    (*Case)[DefAtt]._attr_type = MYCONTRANGE;
                }
                else{
                    ReturnVal._discr_val = XStack[0]._elt_value._discr_val;
                    (*Case)[DefAtt]._attr_type = MYDISC;
                }
                (*Case)[DefAtt]._attr_val = ReturnVal;
                return;
        }
    }
}
