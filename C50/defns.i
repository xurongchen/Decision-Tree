...
//chomimi101
//#include "cpp_wrapper/stack.h"
#include "cpp_wrapper/sort_rec_list.h"
...
typedef struct _cont_range_val{
    ContValue _s_val; // start value
    ContValue _e_val; // end value
}ContRangeVal;
...
typedef struct _attribute_value_and_type{
      AttValue _attr_val;
      AttType _attr_type;
     // CaseNo  _case_no;
}AttValueAndType, *DataRec;
...
#define  CVal(Case,Att)		Case[Att]._attr_val._cont_val
#define  CRSVal(Case,Att)	Case[Att]._attr_val._contran_val._s_val
#define  CREVal(Case,Att)	Case[Att]._attr_val._contran_val._e_val
#define  DVal(Case,Att)		Case[Att]._attr_val._discr_val
#define  XDVal(Case,Att)	(Case[Att]._attr_val._discr_val & 077777777)
#define  SVal(Case,Att)		Case[Att]._attr_val._discr_val
#define  Class(Case)		(*Case)._attr_val._discr_val
#define  Weight(Case)		(*(Case-1))._attr_val._cont_val
#define  TypeVal(Case,Att)   Case[Att]._attr_type
//#define  SizeOfVal(Case,Att) sizeof(Case[Att])
...
typedef	 struct _environment
	 {
      ...
      struct array  *RangeMap; /*map for recording range splity*/
...
/*chomimi101: ProcessWithRangeData.c */
...
void numImplicationsCutAtLeftRightSepWithRange(int i, int Fp, int Lp, struct cmap const *Implications, struct array const *classAttr,
                                               SortRec const *SRect, struct array const *rangeImplications, int rangeCount,
                                               int *numRangeImplicationsCutLeft2Right, int *numRangeImplicationsCutRight2Left,
                                               int *RangeMap, int *numImplicationsCutLeft2Right, int *numImplicationsCutRight2Left);
