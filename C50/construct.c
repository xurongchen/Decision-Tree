...
/*   --------------------  */
{
    ...
    ForEach(Trial, 0, TRIALS - 1) {
        ...
        } else if (Trial && NoStructure || ErrWt / Cases >= 0.49) {
...
/*   -----------------  */
{
    ...
    if (CostWeights) {
        ...
        //modified by chomimi101 for computing range data weights
        ForEach(i, 0, MaxCase) {
            weight = 1.0;
            ForEach(Att, 1, MaxAtt){
                if(TypeVal(Case[i], Att) == MYCONTRANGE && !AttDef[Att]){
                    weight *= (CREVal(Case[i], Att) - CRSVal(Case[i], Att)) + 1;
                }
            }
            Weight(Case[i]) = weight;
        }
        UnitWeights = true;
    }
            ...
            Verbosity(1,
                      PredClass = TreeClassify(Case[i], Raw[0]);
                              ...
                              if (RealClass > 0 && PredClass != RealClass) {
                                  ...
                                  // printf("Erroneous point: %f, %f, %f\n", Case[i]._attr_val[1]._cont_val, Case[i]._attr_val[2]._cont_val,
                                  //Case[i]._attr_val[3]._cont_val);
