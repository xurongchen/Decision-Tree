...
/*   -------  */
{
    ...
    while ((DVec = GetDataRec(Df, Train))) {
        ...
        if (AllowUnknownClass || (Class(DVec) & 077777777) > 0) {
            Case[MaxCase] = DVec;
            MaxCase++;
            // no need to do this in there, for both are global variable
            // actually, it's useful when we allow unknow class example to be added into this set
        } else {
            if (FirstIgnore && Of) {
...
int GetDataRecNew(FILE *Df, Boolean Train)
/*      ----------  */
{
    Attribute Att;
    char Name[1000], *EndName, *MidName;
    int Dv, Chars;
    DataRec DVec;
    ContValue Cv;
    ...
    if (ReadName(Df, Name, 1000, '\00')) {
        ...
        // for range case
        //DVec->_case_no = MaxCase; // MaxCase + 1
        ForEach(Att, 1, MaxAtt) {
            if (AttDef[Att]) {
                DVec[Att]._attr_val = EvaluateDef(AttDef[Att], DVec);

                // may be need to change implementation of check value
                if (Continuous(Att) && TypeVal(DVec, Att) == MYCONT) {
                    CheckValue(DVec, Att);
                }
            ...
            if (Exclude(Att)) {
                ...
                if (TStampVal(Att)) {
                    ...
                    if(Name[0] == '('){
                        ContValue rs = strtod(Name+1, &EndName);
                        if(EndName == Name+1 || *EndName != ':'){
                            XError(BADATTVAL, AttName[Att], Name);
                            DVal(DVec, Att) = UNKNOWN;
                        }else{
                            MidName = EndName+1;
                            ContValue  re = strtod(MidName, &EndName);
                            if(EndName == Name+1 || *EndName != ')'){
                                XError(BADATTVAL, AttName[Att], Name);
                                DVal(DVec, Att) = UNKNOWN;
                            }else{
                        ...
                        TypeVal(DVec, Att) = MYCONTRANGE;
                        //CheckValue(DVec, Att);not check , if check I need modify
                        continue;
                    }else {
...
/*      ----------  */
{
    ...
    if (ReadName(Df, Name, 1000, '\00')) {
        ...
        ForEach(Att, 1, MaxAtt) {
            if (AttDef[Att]) {
                //DVec[Att]._attr_val = EvaluateDef(AttDef[Att], DVec);
                EvaluateDefNew(AttDef[Att], &DVec, Att);
