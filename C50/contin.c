...
/*   -----------------  */
{
    ...
    double LowInfo, LHInfo, LeastInfo = 1E38,
            w, BestGain = NegInfinity, BestInfo = 1E38, ThreshCost = 1, penalty = 0;
    ClassNo c, updateC;
    ContValue Interval, Previous;
    int numImplicationsCut = 0, totalImplications;
    int numImplicationsCutLeft2Right = 0, numImplicationsCutRight2Left = 0;
    int rangeNumImplicationsCutLeft2Right = 0, rangeImplicationsCutRight2Left = 0;
    double reductionL2R, reductionR2L;
    float updateRangeWeight = 0.0;
    struct array *rangeSampleIndexes, *rangeImplicationsIndexes;
    int intervals, index, rangeImplicationsCount = 0;
    float currentRangePreviousWeight,currentRangeAddedWeight;
    //struct IndexSortRec *poppedSortData;
    ...
    /*  Repeatedly check next possible cut  */
    struct sort_rec_list *srecList =  new_sort_rec_list();
    rangeImplicationsIndexes = new_array(2 * MaxCase + 1);
    Previous = GEnv.SRec[GEnv.Xp].V;
    ForEach(i, Fp, Lp) {
        printf(" index %d 's type is %d. \n", i, GEnv.SRec[i].T);// 1 for cont, 3 for cont range
    }
    for (i = GEnv.Xp; i < GEnv.Ep; i++) {
        printf("start iter\n");
        c = GEnv.SRec[i].C;
        ...
        //every time when cut value increase, will add example counts in the left of the cut, will decrease example counts in the right of the cut
        updateRangeWeight = (GEnv.SRec[i].V - Previous) * sort_rec_list_size(srecList);

        // Chomimi101: for range data
        if(GEnv.SRec[i].T == MYCONTRANGE){
            //!FloatEqual(sort_rec_list_back(srecList)->data->V + GEnv.SRec[i].V, 0)
            if(sort_rec_list_empty(srecList) || GEnv.RangeMap->entries[get_sort_rec_index(sort_rec_list_back(srecList))] != i) {
                printf("Push index %d to sort rec list.\n", i);
                sort_rec_list_push_back(srecList, &GEnv.SRec[i], i);
            }
            else{
                printf("Pop index %d from sort rec list.\n", get_sort_rec_index(sort_rec_list_back(srecList)));
                sort_rec_list_pop_back(srecList);
            }
        }

        // now update all the range data's weights, which represents example counts
        printf("update weights.\n");
        if(!sort_rec_list_empty(srecList)){
            rangeSampleIndexes = sort_rec_list_to_array(srecList);
            printf(" i is %d, i's value is %f \n", i,  GEnv.SRec[i].V);
            for(j = 0; j < rangeSampleIndexes->size; j++) {
                index = rangeSampleIndexes->entries[j];
                printf(" index is %d, index's value is %f, index's weight is %f, \n", index,  GEnv.SRec[index].V, GEnv.SRec[index].W);
                //updateC = GEnv.SRec[index].C;
                currentRangePreviousWeight = GEnv.SRec[index].W;
                //intervals =  GEnv.SRec[index].V - GEnv.SRec[index].V + 1;
                intervals =  GEnv.SRec[i].V - GEnv.SRec[index].V + 1.0;
                printf("intervals %f \n", intervals);
                if(intervals == 1.0) {
                    printf("index test 1 %d \n", GEnv.RangeMap->entries[index]);
                    printf("another values is %f, ori range %f \n", GEnv.SRec[GEnv.RangeMap->entries[index]].V  ,GEnv.SRec[GEnv.RangeMap->entries[index]].V - GEnv.SRec[index].V + 1.0);
                    GEnv.SRec[index].W = intervals / (GEnv.SRec[GEnv.RangeMap->entries[index]].V - GEnv.SRec[index].V + 1) * GEnv.SRec[index].W;
                }
                else{
                    printf("index test 2 %d \n", GEnv.RangeMap[index]);
                    printf("previous range %f \n", Previous - GEnv.SRec[index].V + 1.0);
                    GEnv.SRec[index].W = intervals / (Previous - GEnv.SRec[index].V + 1.0) * GEnv.SRec[index].W;
                }
                //(GEnv.SRec[GEnv.RangeMap[index]].V - GEnv.SRec[i].V + 1)
                GEnv.SRec[GEnv.RangeMap->entries[index]].W = (GEnv.SRec[GEnv.RangeMap->entries[index]].V - GEnv.SRec[i].V) / intervals * GEnv.SRec[index].W;

                currentRangeAddedWeight = GEnv.SRec[index].W - currentRangePreviousWeight;
                if(currentRangeAddedWeight > 0 && i != index){//&& i != index
                    if(c > 0){
                        GEnv.LowCases += currentRangeAddedWeight;
                    }
                    GEnv.Freq[2][c] += currentRangeAddedWeight;
                    GEnv.Freq[3][c] -= currentRangeAddedWeight;
                }
                if(i == index){
                    w = GEnv.SRec[i].W;
                }
        ...
        printf("update weights success.\n");
        //GEnv.SRec[i].W = w; // update the weight
        ...
        GEnv.Freq[2][c] += w;
        GEnv.Freq[3][c] -= w;
        if(hasImplicationsWithRangeData(i, GEnv.Xp, GEnv.Ep, GEnv.Implications, GEnv.classAttr, GEnv.SRec) == 1){
            rangeImplicationsIndexes->entries[rangeImplicationsCount++] = i;
        }
//        numImplicationsCutAtLeftRightSep(i, GEnv.Xp, GEnv.Ep, GEnv.Implications, GEnv.classAttr,
//                                         &numImplicationsCutLeft2Right, &numImplicationsCutRight2Left);
        printf("Cut implications.\n");
        numImplicationsCutAtLeftRightSepWithRange(i, GEnv.Xp, GEnv.Ep, GEnv.Implications, GEnv.classAttr, GEnv.SRec,
                                                  rangeImplicationsIndexes, rangeImplicationsCount,
                                                  &rangeNumImplicationsCutLeft2Right, &rangeImplicationsCutRight2Left,GEnv.RangeMap,
                                                  &numImplicationsCutLeft2Right, &numImplicationsCutRight2Left);
        /*if (GEnv.SRec[i].V > 10 || GEnv.SRec[i].V < -10)
            continue;
        */

        // chomimi101: to do, there must be modified
        ...
        if (i == GEnv.Ep || GEnv.SRec[i + 1].V > GEnv.SRec[i].V) {
            ...
            penalty = (1 - nleft * pright) * numImplicationsCutLeft2Right +
                      (1 - nright * pleft) * numImplicationsCutRight2Left;
        ...
        Previous = GEnv.SRec[i].V; // not needed ,previous can be replaced by i-1 indexed, to do more modifications
    }
...
/*   -----------------  */
{
    ...
    /* Find totalImplications that lie entirely between GEnv.Xp and GEnv.Ep */
    numImplicationsOnRight = totalImplications = computeTotalImplications(GEnv.Xp, GEnv.Ep, GEnv.Implications,
                                                                          GEnv.classAttr);
...
/*   -----------------  */
{
    ...
    for (i = GEnv.Xp; i < GEnv.Ep; i++) {
        ...
        // Alg 2
        numImplicationsCutAccordingToAlg2 += numImplicationsCutAt(i, GEnv.Xp, GEnv.Ep, GEnv.Implications,
                                                                  GEnv.classAttr);
        // end
...
int* rangmap_copy_and_rename(struct array  *map, struct array  *permutation){
    struct array  *temp  = new_array(2 * MaxCase + 1);
    for(int i = 0; i < map->size; ++i){
        temp->entries[permutation->entries[i]] = permutation->entries[map->entries[i]];
        temp->entries[permutation->entries[map->entries[i]]] = permutation->entries[i];
    }
    delete_array(map);
    return  temp;
}
...
/*   ----------------  */
{
    ...
    ClassNo c;
    DiscrValue v;
    struct cmap *dynamicImplications;
    /*  Reset frequency tables  */
    ...
    j = Lp;
    dynamicImplications = cmap_copy(Implications);
    printf("\n first index is %d, last index is %d.\n"
            , Fp, Lp);
    printf("before, j is %d.\n", j);
    ForEach(i, Fp, Lp) {
        // modified by chomimi101 for range data
        if(TypeVal(Case[i], Att) == MYCONTRANGE){
            GEnv.SRec[i].V = CRSVal(Case[i], Att);
            ++j;
            GEnv.SRec[j].V = CREVal(Case[i], Att);
            GEnv.SRec[j].W = 0;//Weight(Case[i]),change to 0, for afterwards implications computing
            GEnv.SRec[j].C = Class(Case[i]);
            GEnv.SRec[i].T = MYCONTRANGE;
            GEnv.SRec[j].T = MYCONTRANGE;
            GEnv.permutation->entries[j] = j; // to do check
            GEnv.RangeMap->entries[i] = j;
            GEnv.RangeMap->entries[j] = i;
            printf("Range Map i to j is %d to %d \n", i, j);
            AddImplForRangeWhenSplit(dynamicImplications, i, j, GEnv.permutation);
        }else {
            GEnv.SRec[i].V = CVal(Case[i], Att);
            GEnv.SRec[i].T = MYCONT;
            GEnv.RangeMap->entries[i] = i;
        }
        GEnv.SRec[i].W = Weight(Case[i]);
        GEnv.SRec[i].C = Class(Case[i]);
    ...
    CachesortWithImplications(GEnv.Xp, GEnv.Ep, GEnv.SRec, GEnv.permutation);
    struct array *inverse_permutation = array_invert(GEnv.permutation);

//    ForEach(i, Fp, GEnv.Ep) {
//        printf(" index %d 's type is %d. \n", i, GEnv.SRec[i].T);// 1 for cont, 3 for cont range
//    }
    delete_cmap(GEnv.Implications);
    GEnv.Implications = cmap_copy_and_rename(dynamicImplications, inverse_permutation);
    GEnv.RangeMap = rangmap_copy_and_rename(GEnv.RangeMap, inverse_permutation);
#if false
    ...
    delete_cmap(dynamicImplications);
