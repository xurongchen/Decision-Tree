...
/*   ------------------  */
{
    ...
    GEnv.SRec = Alloc(2 * MaxCase + 1, SortRec); // for range data

    // Pranav: Allocate the new *Implications-related* data structures.
    GEnv.Implications = 0;
    GEnv.permutation = new_array(2 * MaxCase + 1); // for range data
    GEnv.classAttr = new_array(2 *MaxCase + 1); // for range data
    GEnv.RangeMap = new_array(2 * MaxCase + 1); // for range spliting info
    if (SUBSET) {
...
void MyFormTree(CaseNo Fp, CaseNo Lp, int Level, Tree *Result) {
    ...
    if (IntervalsUpperBounds == NULL || IntervalsUpperBounds->size == 0) {
         ...
         */
        //CaseNo LowerCase[MaxCase + 1]; // this is wrong I think...
        CaseNo LowerCase[MaxAttVal[FirstSplitAttr]];
        ...
        ForEach (i, 1, MaxAttVal[FirstSplitAttr]) {
            ...
            printf("%s=%d (%s), enabled=%s: [%d, %d]\n", AttName[FirstSplitAttr], i, AttValName[FirstSplitAttr][i],
                   AttValueFound[i] ? "yes" : "no", LowerCase[i], UpperCase[i]);
        ...
        *Result = Node = Leaf(Nil, 0, 0.0, 0.0);
        ...
        ForEach(i, 1, Node->Forks) {
            ...
            if (AttValueFound[i]) {
                FormTree(LowerCase[i], UpperCase[i], Level + 1, &Node->Branch[i], IntervalsUpperBounds->entries[i - 2],
                         IntervalsLowerBounds->entries[i - 2]);
            }
...
/*   --------  */
{
    ...
    *Result = Node =
    ...
    FindAndDisableRedundantAtt(Fp, Lp, upper, lower);
    //Update Implication Relationships and propagate
    //struct cmap * dynamicImplications = cmap_copy(Implications);
    CaseNo eachCase, otherKnownCase;
    int getKnowCases = 0;
    ForEach(eachCase, Fp, Lp) {
        if(Class(Case[eachCase]) == 0){
            ForEach(otherKnownCase, Fp, Lp){
                if(Class(Case[otherKnownCase]) != 0){
                    Attribute  Att;
                    int flag = 1;
                    ForEach(Att, lower, upper) {
                        if(!Skip(Att)){
                            if(Case[otherKnownCase][Att]._attr_type != Case[eachCase][Att]._attr_type && !(Case[otherKnownCase][Att]._attr_type == MYCONTRANGE &&
                                                                                                         Case[eachCase][Att]._attr_type == MYCONT)){
                                flag = 0;
                                break;
                            }
                            if(Case[otherKnownCase][Att]._attr_type == MYCONT && Case[eachCase][Att]._attr_type == MYCONT &&
                                    (CVal(Case[otherKnownCase],Att) != CVal(Case[eachCase],Att))){
                                flag = 0;
                                break;
                            }else if(Case[otherKnownCase][Att]._attr_type == MYCONTRANGE && Case[eachCase][Att]._attr_type == MYCONTRANGE &&
                                    !(CRSVal(Case[otherKnownCase],Att) <= CRSVal(Case[eachCase],Att)
                                    && CREVal(Case[otherKnownCase],Att) >= CREVal(Case[eachCase],Att))){
                                flag = 0;
                                break;
                            }else if(Case[otherKnownCase][Att]._attr_type == MYCONTRANGE && Case[eachCase][Att]._attr_type == MYCONT &&
                                    !(CRSVal(Case[otherKnownCase],Att) <= CVal(Case[eachCase],Att) && CREVal(Case[otherKnownCase],Att) >= CVal(Case[eachCase],Att))){
                                flag = 0;
                                break;
                            }else if(Case[otherKnownCase][Att]._attr_type == MYDISC && (DVal(Case[otherKnownCase],Att) != DVal(Case[eachCase],Att))){
                                flag = 0;
                                break;
                            }
                    ...
                    if(flag == 1){
                        assignClass(eachCase, Class(Case[otherKnownCase]));
                        ++getKnowCases;
                        struct array * oneArray = new_array(1);
                        oneArray->entries[0] = eachCase;
                        cmap_propagate(oneArray, Implications, Class(Case[otherKnownCase]), &assignClass);
                        delete_array(oneArray);
                        oneArray = NULL;
                    }
                }
            }
...
//    }
    ...
    ResetDisableRedundantAtt(Fp, Lp, upper, lower);
}
...
int compareTwoDataRect(const void *a, const void *b){
    DataRec *l = (DataRec *)a;
    DataRec *r = (DataRec *)b;
    // don't need to compare mydisc and myunknow type
    if((*l)->_attr_type != MYCONT || (*l)->_attr_type != MYCONTRANGE || (*r)->_attr_type != MYCONT || (*r)->_attr_type != MYCONTRANGE )
        return 0;

    ContValue l_start, l_end, r_start, r_end;
    if((*l)->_attr_type == MYCONTRANGE){
        l_start = CRSVal((*l), CurrentCmpAtt);
        l_end = CRSVal((*l), CurrentCmpAtt);
    }else{
        l_start = CVal((*l), CurrentCmpAtt);
        l_end = CVal((*l), CurrentCmpAtt);
    }

    if((*r)->_attr_type == MYCONTRANGE){
        r_start = CRSVal((*l), CurrentCmpAtt);
        r_end = CRSVal((*l), CurrentCmpAtt);
    }else{
        r_start = CVal((*l), CurrentCmpAtt);
        r_end = CVal((*l), CurrentCmpAtt);
    }
...
/*************************************************************************/
/*                                                               	 */
/*	//If a att's value all equal the biggest range from fp to lp, then set this att skip*/
/* new policy: if a artt's value has range type but also different class type(mean at least 2 range example )
/*									 */
/*************************************************************************/

void FindAndDisableRedundantAtt(CaseNo Fp, CaseNo Lp, Attribute upper, Attribute lower){
    Attribute  Att;
    CaseNo  i;
    bool  isReduant = false;
    ClassNo classNo = 0;
    ContRangeVal rangeVal = {0, 0};
    CaseNo prevIndex;

    ForEach(Att, lower, upper){
        CurrentCmpAtt = Att;
        qsort((void *)(Case + Fp), Lp - Fp + 1, sizeof(Case[Fp]), compareTwoDataRect);

        //don't need this, we should check redundant att use the same criterion
//        if (AttDef[Att]) {
//            ForEach(i, 1, AttDefUses[Att][0]) {
//                if(Skip(AttDefUses[Att][i])) {
...
//        }else{
            ...
            // because several ranges has overlap relationships, so we don't check the range scope
            ForEach(i, Fp, Lp) {
                if (Case[i][Att]._attr_type == MYCONTRANGE) {
                    if (prevIndex != -1) {
                        if (Class(Case[i]) != Class(Case[prevIndex])) {
                            if (CRSVal(Case[i], Att) <= CREVal(Case[prevIndex], Att))
                                isReduant = true;
                            break;
                        }
                    }
        ...
        memcpy(Case, SaveCase, (MaxCase + 1) * sizeof(DataRec));
    }
}

//compare function for c++ sort
//bool compareTwoDataRec(DataRec *a, DataRec *b, Attribute Att){
//
//    // it's strange can't use pointer, because it's macro define, you should use ()
//    if(CRSVal((*a),Att) != CRSVal((*b), Att)){
//        return CRSVal((*a),Att) < CRSVal((*b), Att);
//    }
//    if(CREVal((*a),Att) != CREVal((*b), Att)){
//        return CREVal((*a),Att) < CREVal((*b), Att);
//    }
//
//    return true;
//}
...
void ResetDisableRedundantAtt(CaseNo Fp, CaseNo Lp, Attribute upper, Attribute lower) {
    Attribute  Att;
    // why need this? ... emmmm, I can't understand myself
//    CaseNo  i;
//    bool  isReduant = false;
//    int count = 0;
//
//    bool *redSingleAtts = AllocZero(upper - lower + 1, Attribute);
//    ForEach(Att, lower, upper){
//        if (AttDef[Att]) continue;
//        isReduant = true;
//        ForEach(i, Fp, Lp){
//            if(Case[i][Att]._attr_type != MYCONTRANGE || CRSVal(Case[i], Att) != MINCONT || CREVal(Case[i], Att) != MAXCONT){
//                isReduant = false;
//                break;
//            }
//        }
...
//        if (AttDef[Att]){
//            ForEach(i, 1, AttDefUses[Att][0]) {
//                if(redSingleAtts[AttDefUses[Att][i]]) {
//                    SpecialStatus[Att] -= SKIP;
//                    break;
//                }
//            }
//        }
//    }
//    FreeUnlessNil(redSingleAtts);
//    redSingleAtts = Nil;

    // because the default setting is all atts are not skip, so just rest skiped att unskip
    ForEach(Att, lower, upper) {
        if(Skip(Att))
            SpecialStatus[Att] -= SKIP;
    }
